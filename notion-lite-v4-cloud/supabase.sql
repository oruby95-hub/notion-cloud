...
create table if not exists pages (
  ...
  name text not null default '工作區',
  title text,
  created_at timestamptz not null default now()
);
...
alter table project_rows add column if not exists owner text not null default '';
...
update pages set title = coalesce(nullif(title,''), nullif(name,''), '工作區') where title is null or title='';
