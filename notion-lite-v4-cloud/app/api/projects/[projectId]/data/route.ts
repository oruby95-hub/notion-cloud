...
if(b.action==='createPage')({data,error}=await s.from('pages').insert({project_id:projectId,title:b.title||'新頁面'}).select().single());
else if(b.action==='deletePage')({data,error}=await s.from('pages').delete().eq('id',b.id).eq('project_id',projectId));
else if(b.action==='updatePage')({data,error}=await s.from('pages').update({title:String(b.title||'')}).eq('id',b.id).eq('project_id',projectId).select().single());
...
else if(b.action==='createRow')({data,error}=await s.from('project_rows').insert({project_id:projectId,section_id:b.sectionId,item:'新事項',status:'待確認',sort_order:b.sortOrder||0}).select().single());
else if(b.action==='deleteRow')({data,error}=await s.from('project_rows').delete().eq('id',b.id).eq('project_id',projectId));
else if(b.action==='updateRow')({data,error}=await s.from('project_rows').update({item:b.item??'',owner:b.owner??'',progress:b.progress??'',status:b.status??'待確認',start_date:b.start_date||null,due_date:b.due_date||null}).eq('id',b.id).eq('project_id',projectId).select().single());
