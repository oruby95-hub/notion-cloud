...
export async function POST(req: Request, { params }: { params: Promise<{ projectId: string }> }) {
  try {
    ...
    if (action === 'createSection') {
      ...
      const sectionInsert: Record<string, any> = { project_id: projectId, page_id: pageId, name: String(b.name || '新分類'), sort_order: Number(b.sortOrder || 0) }
      if (b.clientId) sectionInsert.id = String(b.clientId)
      const r = await s.from('sections').insert(sectionInsert).select().single()
    ...
    if (action === 'createRow') {
      const rowInsert: Record<string, any> = {
      ...
      if (b.clientId) rowInsert.id = String(b.clientId)
      const r = await s.from('project_rows').insert(rowInsert).select().single()
      if (r.error) return NextResponse.json({ error: r.error.message }, { status: 500 })
