...
export default function Home(){
 ...
 const [hoverText,setHoverText]=useState<{title:string,text:string}|null>(null)
...
{visibleColumns(s.id).map(key=>{
 ...
 if(key==='project_need') return <td key={key} className="expandCell"><div className="expandCellInner" onMouseEnter={()=>setHoverText({title:'專案需求',text:r.project_need||''})} onMouseLeave={()=>setHoverText(null)}><textarea value={r.project_need||''} onChange={e=>updateRow(r,{project_need:e.target.value})}/></div></td>
 if(key==='owner') return <td key={key}><input value={r.owner||''} onChange={e=>updateRow(r,{owner:e.target.value})}/></td>
 if(key==='progress') return <td key={key} className="expandCell"><div className="expandCellInner" onMouseEnter={()=>setHoverText({title:'目前執行狀況',text:r.progress||''})} onMouseLeave={()=>setHoverText(null)}><textarea value={r.progress||''} onChange={e=>updateRow(r,{progress:e.target.value})}/></div></td>
...
</tr>)}</tbody></table></div></section>})}</article>}{hoverText&&<div className="hoverExpand"><div className="hoverExpandTitle">{hoverText.title}</div><div className="hoverExpandText">{hoverText.text||'（尚未填寫）'}</div></div>}{toast&&<div className="toast">{toast}</div>}</section></main>
}
