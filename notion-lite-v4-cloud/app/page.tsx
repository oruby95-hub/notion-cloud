...
export default function Home(){
 ...
 const [dragWorkspace,setDragWorkspace]=useState<string>('')
 const [workspaceOrder,setWorkspaceOrder]=useState<string[]>([])
 useEffect(()=>{try{const x=localStorage.getItem('nc-workspace-order');if(x)setWorkspaceOrder(JSON.parse(x))}catch{}},[])
 useEffect(()=>{if(workspaceOrder.length)localStorage.setItem('nc-workspace-order',JSON.stringify(workspaceOrder))},[workspaceOrder])
 const orderedProjects=useMemo(()=>{
   if(!workspaceOrder.length)return projects
   const map=new Map(projects.map((x:any)=>[x.id,x]))
   const ordered=workspaceOrder.map(id=>map.get(id)).filter(Boolean)
   projects.forEach((x:any)=>{if(!workspaceOrder.includes(x.id))ordered.push(x)})
   return ordered
 },[projects,workspaceOrder])
 const moveWorkspace=(fromId:string,toId:string)=>{
   if(!fromId||!toId||fromId===toId)return
   setWorkspaceOrder(prev=>{
     const base=prev.length?prev:[...projects.map((x:any)=>x.id)]
     const a=base.indexOf(fromId),b=base.indexOf(toId)
     if(a<0||b<0)return base
     const next=[...base];next.splice(a,1);next.splice(b,0,fromId)
     return next
   })
 }
 ...
 const resizeRef=useRef<{sid:string;key:string;startX:number;startW:number}|null>(null)
 useEffect(()=>{try{const x=localStorage.getItem('nc-column-widths');if(x)setColumnWidths(JSON.parse(x))}catch{}},[])
 useEffect(()=>{localStorage.setItem('nc-column-widths',JSON.stringify(columnWidths))},[columnWidths])
 useEffect(()=>{const mv=(e:MouseEvent)=>{const r=resizeRef.current;if(!r)return;setColumnWidths(p=>({...p,[r.sid]:{...(p[r.sid]||{}),[r.key]:Math.max(70,r.startW+e.clientX-r.startX)}}))};const up=()=>{resizeRef.current=null;document.body.style.cursor=''};window.addEventListener('mousemove',mv);window.addEventListener('mouseup',up);return()=>{window.removeEventListener('mousemove',mv);window.removeEventListener('mouseup',up)}},[])
 ...
 const widthOf=(sid:string,key:string)=>columnWidths[sid]?.[key]
 ...
 const addRow=(sec:S)=>{
  const tempId=crypto.randomUUID();
  mutate({action:'createRow',sectionId:sec.id,sortOrder:sec.rows.length,status:statuses[0]||'待確認',clientId:tempId},cur=>({...cur,pages:cur.pages.map(p=>({...p,sections:p.sections.map(x=>x.id===sec.id?{...x,rows:[...x.rows,{id:tempId,section_id:sec.id,item:'新事項',project_need:'',owner:'',progress:'',status:statuses[0]||'待確認',start_date:null,due_date:null,files:[]}]}:x)}))}),'add-row:'+sec.id,'事項已新增',(cur,res)=>({...cur,pages:cur.pages.map(p=>({...p,sections:p.sections.map(x=>({...x,rows:x.rows.map(r=>r.id===tempId?{...r,...res,id:res.id,section_id:res.section_id}:r)}))}))}))
 }
 ...
 const [hoverText,setHoverText]=useState<{title:string,text:string,x:number,y:number}|null>(null)
...
{visibleColumns(s.id).map(key=>{
 ...
 return null
})}<td className="rowDeleteCell"><button className="danger small" onClick={()=>delRow(r.id,s.id)}>刪除</button></td>
</tr>)}</tbody></table></div></section>})}</article>}{hoverText&&<div className="hoverExpand" style={{left:Math.min(hoverText.x+16,window.innerWidth-540),top:Math.min(hoverText.y+16,window.innerHeight-320)}}><div className="hoverExpandTitle">{hoverText.title}</div><div className="hoverExpandText">{hoverText.text||'（尚未填寫）'}</div></div>}{toast&&<div className="toast">{toast}</div>}</section></main>
}
