...
export default function Home(){
 ...
 const addSection=()=>{
  ...
  mutate({action:'createSection',pageId,name:'新分類',sortOrder:sections.length}, cur=>{
    const pages=cur.pages.map((p,i)=>i===0?{...p,sections:[...p.sections,{id:tempId,name:'新分類',rows:[]}]}:p);
    return {...cur,pages};
  },'add-section','分類已新增');
 }
 ...
 const addRow=(s:S)=>{
  const tempId='temp-'+Date.now();
  mutate({action:'createRow',sectionId:s.id,sortOrder:s.rows.length},cur=>{
    const pages=cur.pages.map(p=>({...p,sections:p.sections.map(sec=>sec.id===s.id?{...sec,rows:[...sec.rows,{id:tempId,section_id:s.id,item:'新事項',project_need:'',owner:'',progress:'',status:statuses[0]||'待確認',start_date:null,due_date:null,files:[]}]}:sec)}));
    return {...cur,pages};
  },'add-row:'+s.id,'事項已新增');
 }
