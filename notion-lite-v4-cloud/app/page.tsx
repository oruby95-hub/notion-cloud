...
export default function Home(){
 ...
 const manageStatuses=()=>{const list=statuses.map((s,i)=>`${i+1}. ${s}`).join('\n');const action=prompt(`目前狀態：\n${list}\n\n輸入「新增」、「改:狀態名稱」或「刪:狀態名稱」`);if(!action)return;if(action.trim()==='新增')return addStatus();if(action.startsWith('改:'))return renameStatus(action.slice(2).trim());if(action.startsWith('刪:'))return deleteStatus(action.slice(2).trim())}
